...
import ABTestVariantsTab from './analysis/ABTestVariantsTab';
...
const ComprehensiveResults = ({ results, adCopy, platform, onBack, onFurtherImprove }) => {
  ...
  const [overviewSubTab, setOverviewSubTab] = useState('original');
...
`;
  ...
  const renderOverviewTab = () => (
    <Box>
      ...
      {/* Overview Sub-Tabs: Original | Improved | A/B/C */}
      <Card elevation={0} sx={{ mb: 2, border: '1px solid', borderColor: 'divider' }}>
        <CardContent sx={{ p: 1.5 }}>
          <Tabs
              value={overviewSubTab}
              onChange={(e, v) => setOverviewSubTab(v)}
              variant="scrollable"
              scrollButtons="auto"
              sx={{
                minHeight: 38,
                '& .MuiTab-root': { minHeight: 38, py: 0.5, textTransform: 'none', fontWeight: 600, fontSize: '0.8rem' }
              }}
            >
              <Tab value="original" label="Original" />
              <Tab value="improved" label="Improved" />
              <Tab value="abc" label={`A/B/C Tests (${results.abTests.abc_variants.length})`} />
            </Tabs>
            <Box sx={{ mt: 2 }}>
              {overviewSubTab === 'original' && (
              ...
              {overviewSubTab === 'abc' && (
                (() => {
                  const rawAbc = results.abTests?.abc_variants || [];
                  const fromVariations = (results.abTests?.variations || []).filter(v => {
                    const id = v.id || '';
                    const ver = (v.version || '').toString().toUpperCase();
                    return id === 'variant_a' || id === 'variant_b' || id === 'variant_c' || ['A','B','C'].includes(ver);
                  }).map(v => ({
                    id: v.id || (v.version ? `variant_${v.version.toLowerCase()}` : undefined),
                    version: (v.version || (v.id ? v.id.slice(-1) : '')).toString().toUpperCase(),
                    type: v.type || 'benefit_focused',
                    variant_label: v.variant_label || `VERSION ${(v.version || (v.id ? v.id.slice(-1) : '')).toString().toUpperCase()}`,
                    variant_name: v.variant_name || (v.version === 'A' ? 'Benefit-Focused' : v.version === 'B' ? 'Problem-Focused' : 'Story-Driven'),
                    variant_description: v.variant_description || '',
                    headline: v.headline || '',
                    body_text: v.body_text || v.copy || '',
                    cta: v.cta || '',
                    best_for: v.best_for || [],
                    target_audience: v.target_audience || '',
                    emotional_trigger: v.emotional_trigger || '',
                    psychological_framework: v.psychological_framework || ''
                  }));
                  const abc = rawAbc.length ? rawAbc : fromVariations;
                  return (
                    <>
                      <ABCTestVariants variants={abc} />
                      <Box sx={{ mt: 2 }}>
      ...
      {results.brandVoice && (
        <Card elevation={0} sx={{ mb: 1.5, border: '1px solid', borderColor: 'primary.main', bgcolor: 'rgba(124, 58, 237, 0.02)' }}>
          <CardContent sx={{ p: 1.5, '&:last-child': { pb: 1.5 } }}>
            <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 1.5, display: 'flex', alignItems: 'center', gap: 0.5, fontSize: '0.85rem', color: 'primary.main' }}>
              <RecordVoiceOver sx={{ fontSize: 16 }} />
              Brand Voice Analysis
            </Typography>
            <Grid container spacing={1} sx={{ mb: 1.5 }}>
              <Grid item xs={6} sm={3}>
                <Box sx={{ textAlign: 'center', p: 1, bgcolor: 'rgba(124, 58, 237, 0.05)', borderRadius: 1 }}>
                  <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block', fontSize: '0.65rem' }}>TONE</Typography>
                  <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.8rem' }}>
                    {results.brandVoice.tone || 'Conversational'} {results.brandVoice.tone === 'Casual' ? '😊' : results.brandVoice.tone === 'Professional' ? '💼' : results.brandVoice.tone === 'Friendly' ? '👋' : '💬'}
                  </Typography>
                </Box>
              </Grid>
              <Grid item xs={6} sm={3}>
                <Box sx={{ textAlign: 'center', p: 1, bgcolor: 'rgba(124, 58, 237, 0.05)', borderRadius: 1 }}>
                  <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block', fontSize: '0.65rem' }}>FORMALITY</Typography>
                  <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.8rem' }}>
                    {results.brandVoice.formality || 'Casual'} {results.brandVoice.formality === 'Formal' ? '🎩' : results.brandVoice.formality === 'Semi-formal' ? '👔' : '👕'}
                  </Typography>
                </Box>
              </Grid>
              <Grid item xs={6} sm={3}>
                <Box sx={{ textAlign: 'center', p: 1, bgcolor: 'rgba(124, 58, 237, 0.05)', borderRadius: 1 }}>
                  <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block', fontSize: '0.65rem' }}>PERSONALITY</Typography>
                  <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.8rem' }}>
                    {results.brandVoice.personality || 'Friendly'} {results.brandVoice.personality === 'Professional' ? '💼' : results.brandVoice.personality === 'Creative' ? '🎨' : results.brandVoice.personality === 'Energetic' ? '⚡' : '😊'}
                  </Typography>
                </Box>
              </Grid>
              <Grid item xs={6} sm={3}>
                <Box sx={{ textAlign: 'center', p: 1, bgcolor: 'rgba(124, 58, 237, 0.05)', borderRadius: 1 }}>
                  <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block', fontSize: '0.65rem' }}>CONSISTENCY</Typography>
                  <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.8rem', color: results.brandVoice.consistency > 80 ? 'success.main' : results.brandVoice.consistency > 60 ? 'warning.main' : 'error.main' }}>
                    {results.brandVoice.consistency || 85}% {results.brandVoice.consistency > 80 ? '✅' : results.brandVoice.consistency > 60 ? '⚠️' : '❌'}
                  </Typography>
                </Box>
              </Grid>
            </Grid>
            {results.brandVoice.learningFromPastAds && (
              <Alert severity="info" sx={{ fontSize: '0.7rem', py: 0.5, '& .MuiAlert-message': { py: 0 } }}>
                🧠 <strong>AI Learning:</strong> Analysis enhanced using your past successful ad patterns
              </Alert>
            )}
          </CardContent>
        </Card>
      ...
      {/* Creative Controls Display */}
      {(results.creativity_level || results.urgency_level || results.emotion_type || results.emoji_level) && (
        <Card elevation={0} sx={{ mb: 1.5, border: '1px solid', borderColor: 'secondary.main', bgcolor: 'rgba(156, 39, 176, 0.02)' }}>
          <CardContent sx={{ p: 1.5, '&:last-child': { pb: 1.5 } }}>
            <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 1.5, display: 'flex', alignItems: 'center', gap: 0.5, fontSize: '0.85rem', color: 'secondary.main' }}>
              <Psychology sx={{ fontSize: 16 }} />
              Creative Controls Applied
            </Typography>
            <Grid container spacing={1}>
              {results.creativity_level && (
                <Grid item xs={6} sm={3}>
                  <Box sx={{ textAlign: 'center', p: 1, bgcolor: 'rgba(156, 39, 176, 0.05)', borderRadius: 1 }}>
                    <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block', fontSize: '0.65rem' }}>CREATIVITY</Typography>
                    <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.8rem' }}>
                      {results.creativity_level}/10 {results.creativity_level >= 8 ? '🎨' : results.creativity_level >= 5 ? '💡' : '📝'}
                    </Typography>
                  </Box>
                </Grid>
              )}
              {results.urgency_level && (
                <Grid item xs={6} sm={3}>
                  <Box sx={{ textAlign: 'center', p: 1, bgcolor: 'rgba(156, 39, 176, 0.05)', borderRadius: 1 }}>
                    <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block', fontSize: '0.65rem' }}>URGENCY</Typography>
                    <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.8rem' }}>
                      {results.urgency_level}/10 {results.urgency_level >= 8 ? '🚨' : results.urgency_level >= 5 ? '⏰' : '📅'}
                    </Typography>
                  </Box>
                </Grid>
              )}
              {results.emotion_type && (
                <Grid item xs={6} sm={3}>
                  <Box sx={{ textAlign: 'center', p: 1, bgcolor: 'rgba(156, 39, 176, 0.05)', borderRadius: 1 }}>
                    <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block', fontSize: '0.65rem' }}>EMOTION</Typography>
                    <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.8rem' }}>
                      {results.emotion_type} {results.emotion_type === 'inspiring' ? '✨' : results.emotion_type === 'urgent' ? '🔥' : results.emotion_type === 'curious' ? '🤔' : results.emotion_type === 'empowering' ? '💪' : '😊'}
                    </Typography>
                  </Box>
                </Grid>
              )}
              {results.emoji_level && (
                <Grid item xs={6} sm={3}>
                  <Box sx={{ textAlign: 'center', p: 1, bgcolor: 'rgba(156, 39, 176, 0.05)', borderRadius: 1 }}>
                    <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block', fontSize: '0.65rem' }}>EMOJI LEVEL</Typography>
                    <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.8rem' }}>
                      {results.emoji_level} {results.emoji_level === 'high' ? '😍🎉✨' : results.emoji_level === 'moderate' ? '😊👍' : results.emoji_level === 'minimal' ? '✓' : 'None'}
                    </Typography>
                  </Box>
                </Grid>
              )}
  ...
  const renderABTestsTab = () => {
    // Normalize strategic A/B/C variants from either abc_variants or variations
    const rawAbc = results.abTests?.abc_variants || [];
    const fromVariations = (results.abTests?.variations || []).filter(v => {
      const id = v.id || '';
      const ver = (v.version || '').toString().toUpperCase();
    ...
    const abc_variants = rawAbc.length ? rawAbc : fromVariations;
    const hasStrategicVariants = abc_variants.length > 0;
    ...
    <Box>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        Know What Works Before You Spend - 3 scientifically-designed variants targeting different psychological triggers
      </Typography>
      
      {/* Always render strategic A/B/C container */}
      <ABCTestVariants variants={abc_variants} />
      
      {/* Traditional variations (for backward compatibility) */}
      {(!hasStrategicVariants && results.abTests?.variations?.length > 0 && results.abTests.variations.some(v => v.copy || v.generated_body_text)) && (
        <>
          <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, mt: 3 }}>
            Traditional Variations ({results.abTests.variations.filter(v => v.copy || v.generated_body_text).length} Ready)
          </Typography>
      
      <Grid container spacing={2}>
        {(results.abTests?.variations || []).filter(v => v.copy || v.generated_body_text).map((variation, index) => (
            ...
            >
              <CardContent sx={{ 
                ...
                <Box display="flex" justifyContent="space-between" alignItems="center" mb={1.5}>
                  ...
                  <Chip
                    label={variation.predictedCTR ? `Est. CTR: ${variation.predictedCTR}%` : 'CTR: Not Available'}
                    size="small"
                    color="info"
                    sx={{ fontWeight: 600 }}
                  />
                ...
                <Paper sx={{ 
                  ...
                  <Typography variant="body2" sx={{ 
                    ...
                    {variation.copy || variation.generated_body_text || 'No content available for this variation'}
                  </Typography>
                </Paper>
                ...
                <Button
                  ...
                  onClick={() => copyToClipboard(
                    variation.copy || variation.generated_body_text || '', 
                    `Variation ${index + 1}`
                  )}
                  sx={{
      ...
      {!hasStrategicVariants && (!results.abTests?.variations || results.abTests.variations.length === 0) && (
        <Box textAlign="center" py={8}>
          <Typography variant="body1" color="text.secondary">
            No A/B test variants available yet.
          </Typography>
        </Box>
      )}
    </Box>
