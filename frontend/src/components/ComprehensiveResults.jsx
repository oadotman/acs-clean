...
`;
  ...
  const renderABTestsTab = () => {
    ...
    try {
      ...
      // Format variations for ABCTestingGrid
      const formattedVariations = alternatives.map((alt, index) => {
        // Determine variant type based on index or properties
      ...
      const improvedCopyData = {
        headline: results?.improved?.headline || results?.improved?.copy?.split('\n')[0] || '',
        body_text: results?.improved?.body_text || results?.improved?.copy || results?.improved || '',
        cta: results?.improved?.cta || '',
        score: results?.improved?.score || 85,
        predicted_score: results?.improved?.score || 85
      };
